...
def loginUser(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            # Ensure the user exists in Django's User model or create one
            ...
            return redirect('/create/')  # Redirect to desired URL after login
    ...
    if request.method == "POST":
        ...
        try:
            ...
            user_data = {
                ...
                "last_name": lname,
                "email": email,
                "uid": user_uid  # Optionally store UID for reference
            }
...
def scrape_flipkart(request, wishlist):
    if request.method=="POST":
        username = request.POST.get("username")
        print("US",username)
    Product_name = []
...
def scrape_amazon(request, wishlist):
    if request.method=="POST":
        username = request.POST.get("username")
        print("US",username)
    print("test")
...
def status_update(request, wishlist_name, product_name, status):
    ...
    if status=="bought":
        item_ref.document(product_name).update({
                'status': 'already bought'
        })
    elif status=="wishtobuy":
        item_ref.document(product_name).update({
                'status': 'wish to buy'
        })
    return redirect('/main/'+wishlist_name+'/')
    
def delete_item(request,wishlist_name,product_name):
        user_id = str(request.user)
        print(wishlist_name,product_name)
        user_ref = db.collection('user').document(user_id)
        wishlist_ref = user_ref.collection('wishlist').document(wishlist_name)
